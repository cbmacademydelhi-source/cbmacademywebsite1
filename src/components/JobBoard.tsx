...
import { Briefcase, MapPin, IndianRupee, ArrowRight, Building2, CheckCircle2, Clock, Globe } from 'lucide-react';
...
export const JobBoard: React.FC<JobBoardProps> = ({ onOpenApply }) => {
  ...
  const filteredJobs = JOB_OPPORTUNITIES.filter((job) => {
    ...
    if (filter === 'DELHI') return job.location.includes('Delhi') || job.location.includes('Gurugram') || job.location.includes('Noida');
    return true;
  });

  return (
    <section id="jobs" className="py-16 lg:py-24 bg-white border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="space-y-3 max-w-2xl">
            ...
            <p className="text-slate-600 text-base leading-relaxed">
              Direct interview calls and placement drives across 500+ top agencies and brands.
            </p>
          </div>
          ...
          {filteredJobs.map((job) => (
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded border border-emerald-100 flex items-center gap-1">
                    <Clock className="w-3 h-3" /> Posted {job.postedDaysAgo}d ago
                  </span>
                </div>

                {/* Role & Company Category */}
                <div>
                  <h3 className="text-lg font-bold text-[#072B57] line-clamp-1">
                    {job.role}
                  </h3>
                ...
                <div className="space-y-2 py-2 border-y border-slate-100 text-xs">
                  <div className="flex items-center justify-between text-slate-700">
                    <span className="flex items-center gap-1.5 text-slate-500">
                      {job.isRemote ? <Globe className="w-4 h-4 text-blue-600" /> : <MapPin className="w-4 h-4 text-rose-500" />}
                      Location:
                    </span>
                  ...
                  <div className="flex items-center justify-between text-slate-700">
                    ...
                    <span className="font-extrabold text-[#072B57] text-sm">{job.salary}</span>
                  </div>
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100">
                <button
                  onClick={() => onOpenApply(`Job Placement: ${job.role}`)}
                  type="button"
                  id={`apply-job-${job.id}`}
                  className="w-full inline-flex items-center justify-center gap-2 bg-[#072B57] hover:bg-[#0c3c78] text-white font-bold text-sm py-2.5 rounded-xl transition-all shadow-sm active:scale-98 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#072B57]"
                >
        ...
        {/* Corporate Hiring Partners Logos Bar */}
        <div className="mt-14 pt-8 border-t border-slate-200 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12 opacity-75 grayscale hover:grayscale-0 transition-all duration-300">
            {['Google Partner Agencies', 'Amazon India', 'Swiggy', 'Zomato Media', 'Dentsu International', 'GroupM', 'Nykaa Growth', 'Flipkart'].map((brand, bIndex) => (
              <span key={bIndex} className="text-sm sm:text-base font-extrabold text-[#072B57] tracking-tight hover:text-[#FF6B00] transition-colors">
                {brand}
              </span>
            ))}
          </div>
